...
public class XRManipulable_OneHand : MonoBehaviour
{
    ...
    {
        Vector3 fPos = currentInteractable.interactable.transform.position;
        Vector3 sPos = (minBound + maxBound) / 2;
    ...
    private void RotateContent()
    {
        ...
        Quaternion newRotation = differenceDirection * originalRotation;
        //store in a vector3 to freeze rotations
        Vector3 newEulerAngles = newRotation.eulerAngles;

        //check rotation axis and freeze the other ones
        if (currentInteractable.interactable.name.Contains("Left") || currentInteractable.interactable.name.Contains("Right"))
            newEulerAngles.x = newEulerAngles.z = 0;
        if (currentInteractable.interactable.name.Contains("Top") || currentInteractable.interactable.name.Contains("Bottom"))
            newEulerAngles.y = newEulerAngles.z = 0;
